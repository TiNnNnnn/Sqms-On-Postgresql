 ...
 * However pg_get_indexdef_string_format() includes the index tablespace.
 * ----------
 */
Datum
 ...
 */
char *
pg_get_indexdef_string_format(Oid indexrelid)
{
...
char *
pg_get_indexdef_columns_format(Oid indexrelid, bool pretty)
{
	int			prettyFlags;
					   ...
					   int prettyFlags, bool missing_ok)
{
	...
	context = deparse_context_for_format(get_relation_name(indrelid), indrelid);
			...
			(!colno || colno == keyno + 1))
		{
			...
			if (OidIsValid(indcoll) && indcoll != keycolcollation)
				appendStringInfo(&buf, " COLLATE %s",
								 generate_collation_name_format((indcoll)));
...
char *
pg_get_partkeydef_columns_format(Oid relid, bool pretty)
{
	int			prettyFlags;
						 ...
						 bool attrsOnly, bool missing_ok)
{
	...
	context = deparse_context_for_format(get_relation_name(relid), relid);
	...
	for (keyno = 0; keyno < form->partnatts; keyno++)
	{
		...
		if (!attrsOnly && OidIsValid(partcoll) && partcoll != keycolcollation)
			appendStringInfo(&buf, " COLLATE %s",
							 generate_collation_name_format((partcoll)));
...
pg_get_partition_constraintdef(PG_FUNCTION_ARGS)
{
	 ...
	 */
	prettyFlags = PRETTYFLAG_INDENT;
	context = deparse_context_for_format(get_relation_name(relationId), relationId);
...
/*
 * pg_get_partconstrdef_string_format
 *
 ...
 */
char *
pg_get_constraintdef_command_format(Oid constraintId)
{
		...
		case CONSTRAINT_CHECK:
			{
				...
				{
					/* relation constraint */
					context = deparse_context_for_format(get_relation_name(conForm->conrelid),
												  conForm->conrelid);
				}
				else
				{
...
pg_get_expr_worker(text *expr, Oid relid, const char *relname, int prettyFlags)
{
	...
	if (OidIsValid(relid))
		context = deparse_context_for_format(relname, relid);
	else
		context = NIL;
...
/* ----------
 * deparse_context_for_format			- Build deparse context for a single relation
 *
 * Given the reference name (alias) and OID of a relation, build deparsing
 * context for an expression referencing only that relation (as varno 1,
 * varlevelsup 0).  This is sufficient for many uses of deparse_expression_format.
 * ----------
 */
List *
deparse_context_for_format(const char *aliasname, Oid relid)
{
	deparse_namespace *dpns;
	RangeTblEntry *rte;
...
/*
 * deparse_context_for_plan_tree_format - Build deparse context for a Plan tree
 *
 ...
 * is not usable until set_deparse_context_plan_format() is applied to it.)
 *
 * In addition to the PlannedStmt, pass the per-RTE alias names
 * assigned by a previous call to select_rtable_names_for_explain_format.
 */
List *
deparse_context_for_plan_tree_format(PlannedStmt *pstmt, List *rtable_names)
{
	deparse_namespace *dpns;
...
/*
 * set_deparse_context_plan_format - Specify Plan node containing expression
 *
 ...
 *
 * Once this function has been called, deparse_expression_format() can be called on
...
/*
 * select_rtable_names_for_explain_format	- Select RTE aliases for EXPLAIN
 *
 ...
 */
List *
select_rtable_names_for_explain_format(List *rtable, Bitmapset *rels_used)
{
	deparse_namespace dpns;
		...
		case T_InferenceElem:
			{
				...
				if (iexpr->infercollid)
					appendStringInfo(buf, " COLLATE %s",
									 generate_collation_name_format(iexpr->infercollid));
				...
				switch (spec->strategy)
				{
					...
					case PARTITION_STRATEGY_RANGE:
						...
						appendStringInfo(buf, "FOR VALUES FROM %s TO %s",
										 get_range_partbound_string_format(spec->lowerdatums),
										 get_range_partbound_string_format(spec->upperdatums));
						break;
...
get_const_collation(Const *constval, deparse_context *context)
{
	...
	if (OidIsValid(constval->constcollid))
	{
		...
		{
			appendStringInfo(buf, " COLLATE %s",
							 generate_collation_name_format(constval->constcollid));
		}
	}
}
						   ...
						   deparse_context *context)
{
			...
			l4, rtfunc->funccolnames)
	{
		...
		if (OidIsValid(attcollation) &&
			...
			appendStringInfo(buf, " COLLATE %s",
							 generate_collation_name_format(attcollation));
...
/*
 * generate_collation_name_format
 *		Compute the name to display for a collation specified by OID
 *
 * The result includes all necessary quoting and schema-prefixing.
 */
char *
generate_collation_name_format(Oid collid)
{
	HeapTuple	tp;
...
/*
 * get_range_partbound_string_format
 *		A C string representation of one range partition bound
 */
char *
get_range_partbound_string_format(List *bound_datums)
{
	deparse_context context;
